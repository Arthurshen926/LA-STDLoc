...
Only the V4 bootstrap uses two CPU workers for maps with at least 5,000 Tracks.
The frozen P8 Track factor remains explicitly on its preregistered serial
producer, so neither its source tuple nor old formal artifacts are changed.
Smaller V4 inputs stay serial because process startup and module loading
dominate.  Workers use the current interpreter with `cwd` and `PYTHONPATH`
bound to the checkout that defines this module; a regression test invokes the
parent from an unrelated temporary directory.  This is not a post-CUDA Python
`fork`: workers start a fresh module and receive no inherited Python/CUDA
objects.
...
than claiming a universal speedup.  The requested full 14,753-Track frozen
Shop slice (261,363 observations) took 25.068 s serial and 17.668 s with two
workers (1.42x), with all 25 fields bitwise equal; the threshold is therefore
retained.  A negative test lets one worker finish and forces the other to fail;
the call fails closed, discards partial shards, and a clean retry is bitwise
exact.
