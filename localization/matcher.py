...
def global_view_mixture_topk(
    ...
    *,
    ...
    """Match one or two appearance prototypes while emitting one Anchor vote.

    All prototypes are scored by one GEMM and immediately aggregated along the
    per-Anchor prototype axis.  Padded prototypes have prior zero.  A K=1 row
    takes its score directly from that GEMM, exactly preserving cosine mode.
    """
    ...
    prototype_scores = (query @ normalized.reshape(-1, prototypes.shape[2]).T).reshape(
        query.shape[0], count, 2
    )
