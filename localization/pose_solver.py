...
def solve_absolute_pose(
    ...
    *,
    ...
    pose_w2c = np.concatenate(
        (pose.Rt, np.asarray([[0, 0, 0, 1]])), axis=0
    ).astype(np.float32)
...
def pose_error(predicted_w2c: np.ndarray, ground_truth_w2c: np.ndarray) -> tuple[float, float]:
    ...
    angle_degrees = float(np.linalg.norm(cv2.Rodrigues(rotation_error)[0]) * 180.0 / math.pi)
    predicted_center = np.linalg.inv(predicted)[:3, 3]
    ground_truth_center = np.linalg.inv(ground_truth)[:3, 3]
    translation_centimeters = float(np.linalg.norm(predicted_center - ground_truth_center) * 100.0)
