...
@dataclass
class NoReferenceValidSupportMaskConfig:
...
@dataclass
class NoReferenceValidSupportMask:
...
class NoReferenceValidSupportMaskBuilder:
    """Builds no-reference valid/support masks for synthetic RGB renders.

    valid_mask is conservative and only removes obvious defects such as blank,
    saturated, or fully dark/bright regions. support_mask is a separate
    localization prior based on local image structure.
    """

    def __init__(self, config: Optional[NoReferenceValidSupportMaskConfig] = None):
        self.config = config or NoReferenceValidSupportMaskConfig()
    ...
    def build(self, image):
        ...
        return NoReferenceValidSupportMask(
...
def _save_score(score, path):
    ...
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    array = (torch.as_tensor(score).detach().cpu().float().clamp(0.0, 1.0).numpy() * 255.0).round().astype(np.uint8)
    Image.fromarray(array, mode="L").save(path)
    return str(path)


def save_mask_bundle_pngs(result, path_prefix):
...
NoReferenceValidMaskConfig = NoReferenceValidSupportMaskConfig
NoReferenceValidMask = NoReferenceValidSupportMask
NoReferenceValidMaskBuilder = NoReferenceValidSupportMaskBuilder
save_no_reference_valid_mask_pngs = save_mask_bundle_pngs
