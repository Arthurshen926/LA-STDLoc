...
\subsection{Wide-Scaffold Self-Localization}

The wide 48K scaffold localizes every mapping view with the same global
retrieval used at deployment. At iteration $s$, known mapping poses label the
current candidates. Its Stage-A reconstruction objective separates:
\begin{equation}
...
\end{equation}
The keep term protects correct high-margin assignments; swap and miss rank a
geometrically valid positive above the selected wrong candidate or retrieve a
positive absent from the current top ranks. Global-attractor suppression,
local peak preservation, and a protected pose-critical set prevent ordinary
match gains from damaging precise correspondences. This 1K-step state is our
A0 baseline and supplies descriptors for the subsequent Track-First stage.
...
\subsection{Compact Self-Localization Refresh}

On the fixed compact topology, a 175-step bounded low-rank metric update uses
complete-positive retrieval, current-map hard outcomes, and trajectory-group
robust weighting. Because candidates are regenerated from current descriptors,
both Stage-A and this compact refresh are online matching loops during offline
reconstruction rather than static post-processing. The deployed map retains a
