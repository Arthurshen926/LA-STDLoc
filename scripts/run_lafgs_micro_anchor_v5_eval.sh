...
BANK_ROOT="${LAFGS_V5_BANK_ROOT:?Set LAFGS_V5_BANK_ROOT}"
RUNS="${LAFGS_V5_RUNS:?Set LAFGS_V5_RUNS as profile:budget entries}"
LABEL_PREFIX="${LAFGS_V5_LABEL_PREFIX:-micro_anchor_v5}"
GPU="${LAFGS_V5_GPU:-1}"
...
for run in $RUNS; do
  profile="${run%%:*}"
  budget="${run##*:}"
  label="$(printf '%s_%s_%04d' "$LABEL_PREFIX" "$profile" "$budget")"
  state="$BANK_ROOT/$profile/micro_anchor_$(printf '%04d' "$budget").pt"
  if [[ ! -f "$state" ]]; then
    state="$BANK_ROOT/$profile/fixed_budget_$(printf '%05d' "$budget").pt"
  fi
  if [[ ! -f "$state" ]]; then
    state="$BANK_ROOT/fixed_budget_$(printf '%05d' "$budget").pt"
  fi
  if [[ ! -f "$state" ]]; then
    state="$BANK_ROOT/identity_${profile}.pt"
  fi
