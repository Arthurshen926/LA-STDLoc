...
# Exact-rank descriptor oracle and constrained follow-up. The 895 mapping
# images train rank budgets; all 182 official-test images are a development
# holdout. Deployment remains native SuperPoint, cosine top-1, and one PnP.
...
BASE_ROOT="/mnt/pool/sqy/stdloc_lafgs_v2_alltrain_20260724/OldHospital/robust32k_all895_bootstrap"
BOOTSTRAP="$BASE_ROOT/bootstrap/0_lafgs_map_state.pt"
LANDMARK_IDS="$BASE_ROOT/bootstrap/sampled_idx.pkl"
LANDMARK_META="$BASE_ROOT/bootstrap/landmark_meta.pt"
...
VISIBILITY="$BASE_ROOT/visibility_32000_native.pt"
PROFILE="${LAFGS_RANK_PROFILE:-unconstrained}"
if [[ "$PROFILE" != "unconstrained" && "$PROFILE" != "constrained" ]]; then
  echo "LAFGS_RANK_PROFILE must be unconstrained or constrained" >&2
  exit 2
fi
RUN_ROOT="${LAFGS_RANK_ORACLE_ROOT:-/mnt/pool/sqy/stdloc_lafgs_v2_rank_budget_20260725}/OldHospital/rank_curriculum_${PROFILE}_s2026_steps1000"
MAP_ROOT="$RUN_ROOT/map"
RESULT_ROOT="$RUN_ROOT/results"
LOG_ROOT="$RUN_ROOT/logs"
CFG_ROOT="$RUN_ROOT/configs"
STDLOC_RESULTS_ROOT="$RUN_ROOT/stdloc_results"
read -r -a EVAL_STEPS <<< "${LAFGS_RANK_EVAL_STEPS:-0 1000}"
...
if [[ ! -f "$MAP_ROOT/1000_lafgs_map_state.pt" ]]; then
  RANK_REGULARIZATION=(
    --trust_weight 0 --weight_decay 0 --max_residual_norm 0
    --native_rank_reference_clean_weight 0
    --no-native_rank_landmark_balance
  )
  if [[ "$PROFILE" == "constrained" ]]; then
    RANK_REGULARIZATION=(
      --trust_weight 0.02 --weight_decay 0 --max_residual_norm 0.05
      --native_rank_reference_clean_weight 2
  ...
  run_logged train "$PYTHON" train_lafgs_map.py \
    ...
    --native_rank_budget_mode --native_rank_temperature 0.03 \
    ...
    --feature_lr 0.0002 "${RANK_REGULARIZATION[@]}" \
    --hypothesis_topk 32 --positive_radius_px 2 --negative_radius_px 6 \
    --validation_ratio 0 --split_mode stratified_temporal_block \
    --split_seed 2026 --train_seed 2026 --steps 1000 \
    --save_steps 250 500 895 1000 --log_interval 50
fi
...
for STEP in "${EVAL_STEPS[@]}"; do
  ...
  if [[ ! -f "$POINTER" ]] || [[ ! -f "$(<"$POINTER")/results_summary.json" ]]; then
    run_logged "eval_${STEP}" "$PYTHON" stdloc.py \
      ...
      --cfg "$CFG" --prefix "lafgs-v2-rank-${PROFILE}-${STEP}" \
...
FINAL_RESULT="$(<"$RESULT_ROOT/1000_test.path")"
