...
V03_FULL_BANK_SOURCE_MODE=${V03_FULL_BANK_SOURCE_MODE:-ignore}
V03_FULL_BANK_NEARBY_AS_POSITIVE=${V03_FULL_BANK_NEARBY_AS_POSITIVE:-0}
V03_CHILD_RESPONSIBILITY_MODE=${V03_CHILD_RESPONSIBILITY_MODE:-none}
V03_CHILD_RESPONSIBILITY_START_ITER=${V03_CHILD_RESPONSIBILITY_START_ITER:-0}
V03_ANCHOR_WEIGHT=${V03_ANCHOR_WEIGHT:-0.01}
V03_LOC_OVERLAY_MODE=${V03_LOC_OVERLAY_MODE:-none}
V03_LOC_OVERLAY_LR=${V03_LOC_OVERLAY_LR:-0.0}
V03_LOC_OVERLAY_ACTIVE_LOGIT=${V03_LOC_OVERLAY_ACTIVE_LOGIT:-0.0}
V03_LOC_OVERLAY_MAX_RESIDUAL_NORM=${V03_LOC_OVERLAY_MAX_RESIDUAL_NORM:-0.0}
V03_LOC_OVERLAY_NORMALIZE=${V03_LOC_OVERLAY_NORMALIZE:-0}
V03_LOC_OVERLAY_REG_WEIGHT=${V03_LOC_OVERLAY_REG_WEIGHT:-0.0}
...
FULL_BANK_ARGS=()
if [[ "$V03_FULL_BANK_NEARBY_AS_POSITIVE" == "1" ]]; then
  FULL_BANK_ARGS+=(--loc_full_bank_nearby_as_positive)
fi
LOC_OVERLAY_ARGS=()
if [[ "$V03_LOC_OVERLAY_NORMALIZE" == "1" ]]; then
...
if ! point_cloud_exists "$V03_MODEL" "$V03_END"; then
  "$PYTHON" train_locaware.py \
    ...
    --loc_full_bank_source_mode "$V03_FULL_BANK_SOURCE_MODE" \
    "${FULL_BANK_ARGS[@]}" \
    --loc_child_responsibility_mode "$V03_CHILD_RESPONSIBILITY_MODE" \
    --loc_child_responsibility_start_iter "$V03_CHILD_RESPONSIBILITY_START_ITER" \
    --loc_overlay_mode "$V03_LOC_OVERLAY_MODE" \
    --loc_overlay_lr "$V03_LOC_OVERLAY_LR" \
    --loc_overlay_active_logit "$V03_LOC_OVERLAY_ACTIVE_LOGIT" \
    --loc_overlay_max_residual_norm "$V03_LOC_OVERLAY_MAX_RESIDUAL_NORM" \
    "${LOC_OVERLAY_ARGS[@]}" \
    --loc_overlay_reg_weight "$V03_LOC_OVERLAY_REG_WEIGHT" \
