...
# M4 full-prior marginal-set ablation. GPU 1 only.
...
BANK_ROOT="$ROOT/micro_anchor_v4_full_prior_marginal_v1"
CONFIG_ROOT="$ROOT/configs"
LOG_ROOT="$ROOT/logs"
RESULT_ROOT="$ROOT/results"
STDLOC_RESULTS_ROOT="$ROOT/stdloc_results"
RUNS="${LAFGS_FULL_PRIOR_MARGINAL_RUNS:-query_saturated:96 query_saturated:128 sequence_tail:128}"
...
for run in $RUNS; do
  profile="${run%%:*}"
  budget="${run##*:}"
  label="$(printf 'full_prior_marginal_%s_%04d' "$profile" "$budget")"
