...
def _sample_depth_at_keypoints(
    depth_source: torch.Tensor, keypoints: torch.Tensor, rows: torch.Tensor | None = None
) -> torch.Tensor:
...
def _track_report(tracks: dict, geometry: dict) -> dict:
    ...
    query_count = int(query_index.max()) + 1 if query_index.numel() else 0
    ...
    return {
        ...
        # This is only a capacity proxy, not a selector prediction.  It asks
        # how many non-Track slots are unavoidable if every broad Track could
        # be used in a fixed-size deployed map.
        "surface_capacity_need_proxy_at_7275": max(7275 - int(broad.sum()), 0),
    }


def _pair_report(sidecar: dict) -> dict:
    pair = sidecar["pair"]
    selection_parallax = torch.as_tensor(
        pair["mapping_point_parallax_median_deg"]
    )
    actual_parallax = torch.as_tensor(
...
def _manifest_arguments(path: Path) -> dict:
...
def _build_report(
    *,
    ...
    track_report = _track_report(result["tracks"], result["track_geometry"])
    frozen_track_report = _track_report(
        frozen["tracks"], frozen["track_geometry"]
    )
    ...
    return {
        ...
        "inputs": {
            ...
            "frozen_track_payload": str(frozen_track_payload_path.resolve()),
        },
    }
...
def main() -> None:
    ...
    build_result = build_cycle_consistent_tracks(
        ...
        pair_minimum_joint_visibility_points=int(
        ...
        minimum_margin=float(
            manifest.get("geometry_teacher_track_min_margin", 0.01)
        ),
        ...
        epipolar_recovered_minimum_margin=float(
            manifest.get(
                "geometry_teacher_track_epipolar_recovered_min_margin", -1.0
            )
        ),
        minimum_track_views=int(manifest.get("geometry_teacher_min_views", 3)),
        require_cycle=bool(
            manifest.get("geometry_teacher_track_require_cycle", True)
        ),
    ...
    observation_uv = all_keypoints[
        offsets[observation_query] + observation_keypoint
    ]
    ...
    geometry = robust_triangulate_associations(
        ...
        minimum_view_bins=int(
            manifest.get("geometry_teacher_min_view_bins", 2)
        ),
        ...
        maximum_rendered_depth_residual_m=float(
            manifest.get(
                "geometry_teacher_max_rendered_depth_residual_m", float("inf")
            )
        ),
