#!/usr/bin/env python3
"""Evaluate the frozen V21 Top-K geometric feedback arm."""
...
from map_learning import v21_topk_geometric_feedback as feedback
...
def _load(path: Path, expected: str) -> tuple[dict, dict]:
    ...
    if digest != expected:
        raise ValueError(f"V21 Top-K source SHA differs: {resolved}")
...
def main() -> None:
    ...
    producers = []
    for path in [Path(feedback.__file__).resolve(), Path(__file__).resolve()]:
        producers.append(
            {
                "path": str(path),
    ...
    for source in [stable_source, *(value[1] for value in caches), *producers]:
        if sha256_file(source["path"]) != source["sha256"]:
            raise RuntimeError(f"V21 Top-K source changed: {source['path']}")
