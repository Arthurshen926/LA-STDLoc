...
def multi_positive_assignment_loss(
    ...
    *,
    ...
    all_logits = torch.cat([candidate_logits, null_logits[:, None]], dim=1)
    denominator = torch.logsumexp(all_logits, dim=1)
    ...
    if bool(has_positive.any().item()):
        ...
        positive_loss = (
            denominator[has_positive] - positive_numerator
        ).mean()
    null_rows = ~has_positive
    null_loss = candidate_logits.sum() * 0.0
    if bool(null_rows.any().item()):
        null_loss = (
            denominator[null_rows] - null_logits[null_rows]
        ).mean()
    if bool(has_positive.any().item()) and bool(null_rows.any().item()):
        loss = 0.5 * positive_loss + 0.5 * null_loss
    else:
        loss = positive_loss + null_loss
    return loss, {
...
def main():
    ...
    for name in tqdm(query_names, desc="One-of-K feature cache"):
        ...
        feature_cache.append(
            (local_features.detach().cpu(), positives.detach().cpu())
        )
        diagnostics.append(
            {
    ...
    for epoch in range(args.epochs):
        ...
        for index in tqdm(order, desc=f"One-of-K train {epoch + 1}/{args.epochs}"):
            ...
            with torch.no_grad():
                ...
                target_has_positive = positives.any(dim=1)
