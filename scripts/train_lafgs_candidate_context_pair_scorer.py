...
def main() -> None:
    ...
    with torch.inference_mode():
        for query_index, name in enumerate(baseline["query_names"]):
            ...
            for local_index, slot in enumerate(changed.tolist()):
                ...
                correct_contexts.append(map_context[correct].half())
                confusing_contexts.append(map_context[confusing].half())
    ...
    for fold_index, trajectory in enumerate(unique_trajectories):
        ...
        calibration_count = max(
            int(round(len(shuffled) * float(args.calibration_fraction))), 1
        )
        ...
        fold_models[trajectory] = _model_payload(
            model, scalar_mean, scalar_scale, threshold
        )
    ...
    generator = np.random.default_rng(args.seed + 1000)
    shuffled = generator.permutation(all_indices)
    calibration_count = max(
        int(round(len(shuffled) * float(args.calibration_fraction))), 1
    )
    calibration = shuffled[:calibration_count]
    ...
    _atomic_torch(
        output_model,
        {
            ...
            "full_model": _model_payload(
                full_model, full_mean, full_scale, full_threshold
            ),
