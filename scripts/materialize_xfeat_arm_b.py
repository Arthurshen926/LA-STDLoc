#!/usr/bin/env python3
"""Materialize a locked CPU-only XFeat descriptor-identity (Arm-B) probe.

This command is offline and mapping-only.  It never runs the XFeat detector,
never consults test images, and refuses a dirty or hash-mismatched external
checkout.  The output is consumed by ``audit_frontend_upper_bound descriptor``.
"""
...
def run(args: argparse.Namespace) -> dict:
    if args.device != "cpu":
        raise ValueError("XFeat Arm B is CPU-only")
    ...
    protected = {query_cache_path, teacher_path, weights_path}
    ...
    # Hash before loading so the exact serialized sources are bound in output.
    expected_query_cache_sha256 = _expected_sha256(
        args.expected_query_cache_sha256,
        label="expected query-cache SHA256",
    )
    expected_teacher_sha256 = _expected_sha256(
    ...
    if query_cache_sha256 != expected_query_cache_sha256:
        raise ValueError(
            "query-cache SHA256 mismatch: "
            f"{query_cache_sha256} != {expected_query_cache_sha256}"
        )
    if teacher_sha256 != expected_teacher_sha256:
        raise ValueError(
            "teacher SHA256 mismatch: "
    ...
    if probe["reference"]["query_cache_sha256"] != query_cache_sha256:
        raise RuntimeError("query cache changed while materializing Arm B")
    if probe["reference"]["teacher_sha256"] != teacher_sha256:
        raise RuntimeError("teacher changed while materializing Arm B")
    if file_sha256(query_cache_path) != query_cache_sha256:
        raise RuntimeError("query cache changed before Arm-B output commit")
    if file_sha256(teacher_path) != teacher_sha256:
        raise RuntimeError("teacher changed before Arm-B output commit")
    ...
    return {
        ...
        "reference_row_count": probe["producer"]["reference_row_count"],
    }
