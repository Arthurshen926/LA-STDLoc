...
def build_base(
    *,
    ...
    for path, label in ((cache, "feature cache"), (track, "Track payload"), (anchor_map, "Track map")):
...
def run_scene(args: argparse.Namespace) -> dict:
    ...
    if not appearance_cache.exists():
        ...
        run_module(
            ...
            arguments=[
                "--dataset", str(dataset), "--images", "processed",
                "--gaussian-ply", str(prior), "--gaussian-type", "2dgs",
                "--sh-degree", "3", "--source-cache", str(base_cache),
                "--track-payload", str(base_track), "--selected-map", str(base_map),
                "--output-dir", str(appearance), "--nms-radius", "4",
                "--alpha-minimum", "0.05", "--alpha-erosion-radius", "4",
                "--descriptor-trim-fraction", "0.2", "--progress-interval", "25",
            ],
            env=env,
        )
    ...
    if not repaired_track.exists():
        ...
        run_module(
            ...
            arguments=[
                "--source-cache", str(base_cache), "--expected-source-cache-sha256", sha256(base_cache),
                "--support-cache", str(appearance_cache), "--expected-support-cache-sha256", sha256(appearance_cache),
                "--source-track-payload", str(base_track), "--expected-source-track-payload-sha256", sha256(base_track),
                "--output-dir", str(support), "--depth-abs-tolerance-m",
                str(initial["parameters"]["evidence_depth_abs_tolerance_m"]),
                "--device", "cuda:0", "--progress-interval", "25",
            ],
            env=env,
        )
    ...
    if not completion.exists():
        run_module(
            ...
            arguments=[
                "--query-cache", str(appearance_cache), "--expected-query-cache-sha256", sha256(appearance_cache),
                "--track-payload", str(repaired_track), "--expected-track-payload-sha256", sha256(repaired_track),
                "--output", str(completion), "--voxel-size-m", str(parameters["surface_group_voxel_m"]),
                "--maximum-candidates", "512", "--maximum-rows-per-view", "256",
                "--alpha-minimum", "0.05", "--minimum-observations", "3", "--minimum-views", "2",
                "--minimum-pose-bins", "2", "--descriptor-trim-fraction", "0.2",
            ],
            env=env,
        )
    ...
    if not graph.exists():
        ...
        run_module(
            ...
            arguments=[
                "--surface-map", str(completion), "--expected-surface-map-sha256", sha256(completion),
                "--query-cache", str(appearance_cache), "--expected-query-cache-sha256", sha256(appearance_cache),
                "--track-payload", str(repaired_track), "--expected-track-payload-sha256", sha256(repaired_track),
                "--output-dir", str(selector_inputs),
            ],
            env=env,
        )
    ...
    if not selected_maps:
        reject_partial(selector, (selector / "scene_calibration.json",), label="selector")
        run_module(
            code_root=code_root,
            scene_root=scene_root,
            stage="selector",
            ...
            arguments=[
                "--canonical-map", str(completion),
                "--function-graph", str(graph),
                "--complete-positive-teacher", str(selector_inputs / "surface_completion_teacher.pt"),
                "--track-payload", str(repaired_track), "--query-cache", str(appearance_cache),
                "--rendered-track-pose-minimum-additions", "0", "--output-dir", str(selector),
            ],
            env=env,
        )
        selected_maps = sorted(selector.glob("adaptive_compact_total*.pt"))
    if len(selected_maps) != 1:
        raise RuntimeError(f"expected exactly one selected map in {selector}, got {selected_maps}")
    ...
    if not training_map.exists():
        ...
        run_module(
            ...
            arguments=[
                "--anchor-map", str(compact_map), "--track-payload", str(repaired_track),
                "--query-cache", str(appearance_cache), "--output-dir", str(training),
                "--strong-radius-px", str(parameters["positive_radius_px"]),
                "--ambiguous-radius-px", str(parameters["negative_radius_px"]),
                "--single-trajectory-pose-cells", "3", "--alpha-minimum", "0.05",
                "--depth-abs-tolerance-m", str(parameters["evidence_depth_abs_tolerance_m"]),
                "--depth-relative-tolerance", "0.02", "--scene-calibration", str(scene_calibration),
                "--expected-scene-calibration-sha256", sha256(scene_calibration),
            ],
            env=env,
        )
    ...
    if not metric.exists():
        run_module(
            ...
            arguments=["--map", str(training_map), "--expected-map-sha256", sha256(training_map), "--output", str(metric)],
            env=env,
        )
    ...
    if not summary_path.exists():
        ...
        run_module(
            ...
            arguments=[
                "--dataset", str(dataset), "--images", "processed", "--map", str(training_map),
                "--metric-state", str(metric), "--scene-calibration", str(scene_calibration),
                "--output", str(evaluation), "--split", "test", "--device", "cuda:0",
                "--seed", str(args.seed),
            ],
            env=env,
        )
    ...
    result = {
        ...
        "artifacts": {
            "base_cache": str(base_cache), "base_cache_sha256": sha256(base_cache),
            "base_track": str(base_track), "base_track_sha256": sha256(base_track),
            "appearance_cache": str(appearance_cache), "appearance_cache_sha256": sha256(appearance_cache),
            "repaired_track": str(repaired_track), "repaired_track_sha256": sha256(repaired_track),
            "selected_map": str(compact_map), "selected_map_sha256": sha256(compact_map),
            "training_map": str(training_map), "training_map_sha256": sha256(training_map),
            "metric": str(metric), "metric_sha256": sha256(metric),
            "scene_calibration": str(scene_calibration), "scene_calibration_sha256": sha256(scene_calibration),
...
def main() -> None:
    ...
    try:
        ...
        atomic_json(root / "single_seed_error.json", {"scene": args.scene, "error": repr(exc)})
        raise
