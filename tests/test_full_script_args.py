...
class FullRunScriptArgsTest(unittest.TestCase):
    ...
    def test_v03_multiscene_script_dispatches_scene_seed_matrix_safely(self):
        ...
        self.assertIn("SEEDS=${SEEDS:-2025 2026 2027}", text)
        ...
        self.assertIn('V03_QUERY_SPLIT_SEED="$seed"', text)
        self.assertIn('MODEL_ROOT="$MODEL_ROOT/${scene}/seed_${seed}"', text)

    def test_v03_topology_script_runs_labelstate_physical_prune_and_remap_eval(self):
        ...
        self.assertIn("--train_phase topology", text)
        self.assertIn("--enable_topology", text)
        self.assertIn("--topology_enable_physical_prune", text)
        self.assertIn("--topology_protect_landmarks", text)
